...
export function EmployeeFormModal({ onClose, onSuccess }: Props) {
  ...
  function handleSubmit() {
    setErrorMsg("");
    mutation.mutate({
      data: { loginId, password, name, email, phone, birth, gender,
    ...
    <BaseModal title="직원 추가" onClose={handleClose} isClosing={isClosing} size="lg"
      footer={
        <div style={{ display: "flex", gap: "8px", justifyContent: "flex-end", width: "100%" }}>
          <Button variant="secondary" type="button" onClick={handleClose} disabled={mutation.isPending}>취소</Button>
          <Button variant="primary" type="button" onClick={handleSubmit} disabled={mutation.isPending}>등록</Button>
        </div>
      }
    >
      <div className={ms.fieldStack}>
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
          ...
          <EmployeeField label="로그인 ID"><input className={ms.fieldBox} value={loginId} onChange={(e) => setLoginId(e.target.value)} placeholder="로그인 ID" /></EmployeeField>
